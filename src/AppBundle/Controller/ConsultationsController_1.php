...
class ConsultationsController extends Controller {
    ...
    public function indexAction() {
        ...
        $entities = $em->getRepository('AppBundle:Consultations')->findAll();
 ...
 /**
    ...
    public function showSharedAction($id, $pat) {
        ...
        if ($this->get('security.context')->isGranted('ROLE_PHYSICIANS')) :
            $entityPhy = $em->getRepository('AppBundle:Physicians')->findOneBy(array('user' => $user->getId()));
    ...
    public function updateAction(Request $request, $id) {
        ...
        if ($editForm->isValid() || (null !== $request->request->get("cancel") || null !== $request->request->get("confirm"))) :
 		...
 		//inicio generar notificacion Alerta
            ...
            return $this->redirect($this->generateUrl('consultations_list_physician'));
        endif;
...
//            Braintree_Configuration::privateKey('3d13f1ac11aa8dfa7535ae6eef17cc97');
     ...
     * Lists all Consultations entities.
     *
     * @Route("/physician/list", name="consultations_list_physician")
     * @Method("GET")
     * @Template()
     * @Security("has_role('ROLE_ADMIN') or has_role('ROLE_PHYSICIANS')")
     */
    public function listPhysicianAction() {
        $em = $this->getDoctrine()->getManager();

        $entityPhy = $this->getPhysicianLog();
        
        if (null===$entityPhy->getDatetimeAvailable() || 'null'===$entityPhy->getDatetimeAvailable() || ''===$entityPhy->getDatetimeAvailable()) {
            return $this->redirect($this->generateUrl('physicians_edit_availability',array('id' => $entityPhy->getUser()->getId())));                   
        } 
    ...
    public function getPhysicianLog() {
        ...
        $entityPat = $em->getRepository('AppBundle:Physicians')->findOneBy(array('user' => $user->getId()));
        if (!$entityPat) {
            throw $this->createNotFoundException('Unable to find Physician.');
        }
